...
3. Перейти в корневую папку проекта для дальнейших действий (команда зависит от текущего расположения, дан пример):
```
cd DRFBooking
```

4. Установить необходимые для запуска зависимости с помощью команды:
```
poetry install --with dev
```

5. Запустить виртуальное окружение для poetry:
```
eval $(poetry env activate)
```

6. Для запуска сервиса, находясь в директории, где расположен `Makefile`, выполнить команду:
```
make dev
```
...
# Тестирование
Для запуска автотестов, находясь в директории, где расположен `Makefile`, выполните команду:
```
make test
```

Примечание: для корректной работы необходимо выполнение пунктов 1-4 раздела "Запуск"
